...
5. **최종 출력 전 자체 중복 검사(반드시 수행):** 워드프레스 각 섹션을 쓴 뒤, 그 섹션의 핵심 문장이 네이버 본론의 문장과 사실상 동일하거나 살짝만 바꾼 재진술인지 스스로 검토한다. 특히 "언제(시기·기간)", "무엇을(대상·정의)"처럼 네이버 개요에서 이미 다룬 배경 정보를 워드프레스 섹션의 핵심 내용으로 다시 서술하고 있다면 그 섹션은 실패다 — 워드프레스는 네이버가 다루지 않은 좁고 구체적인 실행 방법·수치·비교·팁 등으로만 채운다. 배경 정보가 꼭 필요하면 한두 문장으로 짧게만 스치고 지나가되, 그 문장을 섹션의 핵심 내용으로 삼지 않는다.
