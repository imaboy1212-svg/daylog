...
[IMAGE_PROMPT_NAVER_1_1]네이버 카드뉴스용 1:1 정사각형 이미지 프롬프트, 한글로 작성. 아이콘·색상 블록 위주의 추상적 인포그래픽이 아니라, 이 글의 주제를 직관적으로 알아볼 수 있는 **실사 사진 또는 고퀄리티 애니메이션(일러스트) 스타일** 장면을 중앙에 크게 배치하도록 묘사한다(예: 관련 인물·사물·장소를 담은 사진풍 또는 일러스트풍 장면). 그 배경 위에 반투명 단색 컬러 오버레이를 입혀 글자가 잘 보이게 하고, 화면 중앙에 주제(대제목)와 부제(부제목) 텍스트를 큼직하게 배치한다고 명시한다. 실제로 들어갈 제목·부제 문구를 프롬프트에 그대로 포함해 이미지 생성기가 렌더링하도록 요청한다(텍스트 정확도 제약 없음). 워터마크·로고·특정 브랜드/실존 인물 식별 요소는 넣지 않는다[/IMAGE_PROMPT_NAVER_1_1]
