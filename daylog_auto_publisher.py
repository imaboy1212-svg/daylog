...
[IMAGE_PROMPT_NAVER_1_1]네이버 카드뉴스용 1:1 정사각형 이미지 프롬프트, 한글로 작성. **아래 NAVER_TITLE 태그에 쓸 문구(넓은 개요 키워드 기준)를 대제목으로 사용한다 — TITLE(워드프레스 심화 제목)을 재사용하지 않는다.** 아이콘·색상 블록 위주의 추상적 인포그래픽이 아니라, 이 글의 주제를 직관적으로 알아볼 수 있는 **실사 사진 또는 고퀄리티 애니메이션(일러스트) 스타일** 장면을 중앙에 크게 배치하도록 묘사한다(예: 관련 인물·사물·장소를 담은 사진풍 또는 일러스트풍 장면). 그 배경 위에 반투명 단색 컬러 오버레이를 입혀 글자가 잘 보이게 하고, 화면 중앙에 NAVER_TITLE 기반 대제목과 그에 어울리는 부제 텍스트를 큼직하게 배치한다고 명시한다. 실제로 들어갈 제목·부제 문구를 프롬프트에 그대로 포함해 이미지 생성기가 렌더링하도록 요청한다(텍스트 정확도 제약 없음). 이미지 내 모든 텍스트는 고유명사를 제외하고 한글로 표기하며 영문 사용을 금지한다는 문장을 포함한다. 워터마크·로고·특정 브랜드/실존 인물 식별 요소는 넣지 않는다[/IMAGE_PROMPT_NAVER_1_1]
[IMAGE_PROMPT_WP_16_9]워드프레스 대표이미지용 16:9 가로형 이미지 프롬프트, 한글로 작성. **위 TITLE 태그에 쓸 문구(포커스 키워드가 들어간 심화 제목)를 대제목으로 사용한다 — NAVER_TITLE과는 다른 문구여야 한다.** 위와 동일하게 추상적 인포그래픽이 아니라 **실사 사진 또는 애니메이션(일러스트) 스타일**의 주제를 직관적으로 보여주는 장면을 화면 중앙에 크게 배치. 배경에 반투명 단색 컬러 오버레이를 입혀 가독성을 확보하고, 중앙에 TITLE 기반 대제목과 그에 어울리는 부제 텍스트를 큼직하게 배치한다고 명시하며, 실제 문구를 프롬프트에 그대로 포함한다(텍스트 정확도 제약 없음). 이미지 내 모든 텍스트는 고유명사를 제외하고 한글로 표기하며 영문 사용을 금지한다는 문장을 포함한다. 워터마크·로고·특정 브랜드/실존 인물 식별 요소는 넣지 않는다[/IMAGE_PROMPT_WP_16_9]
